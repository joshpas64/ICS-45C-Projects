...
public:
    ...
    void setLine(const std::string &str, int lineNumber);

    // Returns the error message that should be displayed currently.
    const std::string& currentErrorMessage() const;

    void incLine(int lineNumber, const std::string &str); //Inserts line with with a string inside it at specified line
    // Next three methods are just setters for the cursorLine , cursorColumn, and lineCount
    void setCursorLine(int lineNumber); 

    void setCursorCol(int colNumber);

    void setLineCount(int count);

    void decLine(int lineNumber);//Deletes line and its contents at specified lineNumber
...
private:
    ...
    int currentCol;
    int lineTotals;
    std::vector<std::string> lines; //Used to find set, insert, and delete line content and specific places
    std::string currentErrorStatement;
